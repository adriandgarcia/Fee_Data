...
First, I pulled in the data from Morningstar Direct. (I combined it using “Combine_Files.r”). There were 25,569 rows and 89 columns. Data was pulled from Morningstar Direct on July 10, 2020.
...
Active_Cat_Pct %>%
  filter(Firm.Name %in% Lowcost_Active_Firms) %>%
  arrange(-pct_assets_2019, .by_group = TRUE) %>% 
  ungroup() %>% 
...
Passive_Cat_Pct <- Passive_Cat %>%
  select(Firm.Name, US.Category.Group, pct_assets) %>% 
