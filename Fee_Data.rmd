...
Active_MSCat <- ActiveFunds %>% 
  group_by(Firm.Name, Morningstar.Category) %>% 
  summarise(
    Asst_Wted_Cat_2017 = sum(Asst_By_Fee_2017, na.rm = TRUE),
    Sum_Assets_Cat_2017 = sum(Net_Asst_2017_AVG, na.rm = TRUE),
    Fee_Wted_Cat_2017 = Asst_Wted_Cat_2017 / Sum_Assets_Cat_2017,
...
write_csv(Active_MSCat,
          "~/Data/Fee_Data/Fee_Data/results/ACTIVE/MScat_activefunds.csv")
...
Active_MSCat_Sum <- Active_MSCat %>%
  ...
  summarise(
    SHARECLASSES = sum(Cat_Count),
    FEE = round((sum(Asst_Wted_Cat_2019)/sum(Sum_Assets_Cat_2019)),2),
    ASSETS = sum(Sum_Assets_Cat_2019)
  ) %>%  
  mutate(
    pct = round(ASSETS / sum(ASSETS)*100,2),
...
write_csv(Active_MSCat_Pct,
          "~/Data/Fee_Data/Fee_Data/results/ACTIVE/MScat_activefunds_highlow.csv")
